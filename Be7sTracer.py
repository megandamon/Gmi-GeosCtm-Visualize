...
class Be7sTracer(GenericTracer):
    ...
    def __init__(self, tracerName, modelObject, keyFile, yAxisType):
        GenericTracer.__init__(self, tracerName, modelObject, keyFile, timeRecord)

        print (dir(modelObject))

        if "TR_GOCART" in modelObject.fileName or "cycling" in modelObject.fileName:

            print ("Be7s tracer needs specific humidity")
...
#             self.units = "kg kg-1"
        ...
        if "TR_GOCART" in simName or "cycling" in simName:
            ...
            print ("Converting Be7s to mol/mol")
#            convertArray = array *((self.MOL_WEIGHT_DRY_AIR)/(self.MOL_WEIGHT))
    ...
    def createDiffContoursFromMinMax (self, minVal, maxVal):
        ...
        print ("Be7s createDiff received: ", minVal, maxVal)
