        ...
        while lineCount < len(keyLines) - 1:
            ...
            if 'slice' not in tracerMetData[0] and 'z_zm' not in tracerMetData[0]:
                ...
                if tracerName.lower() == "aoa_bl":
                    self.tracerDict[tracerName] = AoaBlTracer(tracerName, modelObject, keyFile, 'linear', timeRecord,
                                                              fileLevel)
                    ...
                    self.tracerDict[tracerName] = GenericTracer(tracerName, modelObject, keyFile, timeRecord,
                                                                fileLevel)  # create new tracer info
        ...
        while lineCount < len(keyLines) - 1:
            ...
            while not nextTracer and lineCount < len(keyLines) - 1:
                ...
                if sliceLine[0] == "\tslice":
                    ...
                    tracerDict[tracerCount]['slices'][sliver] = None
                    if len(sliceLine) > 2:

                        contourLevels = sliceLine[2].split(",")
                        levCount = 0
                        tracerDict[tracerCount]['slices'][sliver] = []
                        for level in range(len(contourLevels)):
                            tracerDict[tracerCount]['slices'][sliver].append(float(contourLevels[levCount]))
                            levCount = levCount + 1
                else:
